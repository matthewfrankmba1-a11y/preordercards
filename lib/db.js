...
function initDb() {
  ...
  // lib/newsletter.js). 'signups' is the consented marketing list — the
  // banner and /newsletter.html forms both say the address is used for
  // newsletters. 'all' additionally pulls people who only ever registered
  // interest in a specific release, which is a broader read of consent, so
  // it's opt-in rather than the default. Both suppress unsubscribes, hard
  ...
  const listNewsletterRecipientsSignups = db.prepare(`
    SELECT DISTINCT email FROM (
      SELECT LOWER(email) AS email FROM discount_signups
  ...
  const listNewsletterRecipientsAll = db.prepare(`
    SELECT DISTINCT email FROM (
      ...
      WHERE welcome_email_status IS NULL OR welcome_email_status <> 'bounced'
      UNION
      SELECT LOWER(contact_value) AS email FROM interests
      WHERE contact_type = 'email' AND cancelled_at IS NULL
      UNION
      SELECT LOWER(contact_value) AS email FROM listing_interests
      WHERE contact_type = 'email' AND cancelled_at IS NULL
    )
    WHERE ${NEWSLETTER_SUPPRESSION}
    ORDER BY email
