...
function initDb() {
  ...
  const insertPreorderRequest = db.prepare(`
    INSERT INTO preorder_requests (product, quantity, contact_type, contact_value, notes)
    VALUES (@product, @quantity, @contactType, @contactValue, @notes)
  `);
  ...
  const listPreorderRequestsForAdmin = db.prepare(`
    SELECT id, product, quantity, contact_type AS contactType, contact_value AS contactValue,
           notes, handled_at AS handledAt, created_at AS createdAt
