...
async function fetchSourceText(source) {
  ...
  try {
    const res = await fetch(source.url, {
      ...
      headers: {
        // Identifies the bot and points at the site, so an operator who sees
        // it in their logs can tell what it is and block it if they'd rather.
        'User-Agent': `PreorderCardsReleaseCheck/1.0 (+${SITE_URL})`,
        Accept: 'text/html,application/xhtml+xml',
      },
    });
    if (!res.ok) return { ok: false, error: `HTTP ${res.status}` };
    const text = htmlToText(await res.text());
    if (text.length < 500) return { ok: false, error: `Page returned only ${text.length} characters of text` };
    return { ok: true, text };
  } catch (err) {
...
async function runReleaseCheck({ notify = true } = {}) {
  ...
  const report = { ranAt: new Date().toISOString(), calendarSize: calendar.length, sources: results };
  if (notify) await notifyDiscord(report);
  return report;
}
