...
async function buildIssue(dateISO, { generate = false } = {}) {
  ...
  const allReleases = getReleasesInWindow(since, until);

  // Anything struck out during this week's date check is dropped before the
  // email is built, so the issue simply doesn't mention it. Omitting a real
  // release is a much smaller error than asserting a wrong date for it, and
  // it takes a click rather than a data edit and a redeploy.
  const excluded = new Set(db.listNewsletterExclusions.all(since).map((row) => row.releaseId));
  const releases = allReleases.filter((r) => !excluded.has(r.id));
  const excludedReleases = allReleases.filter((r) => excluded.has(r.id));
  ...
  return {
    ...
    releases,
    // Not mailed — carried so the admin panel can show what was struck out
...
// --- date confirmation gate ------------------------------------------------

// data/releases.json is compiled by hand from third-party trackers, which
// lag the manufacturer and sometimes disagree with each other. A wrong date
// on the site can be corrected in a minute; a wrong date in an email is in
// someone's inbox forever. So a send is gated on a human having checked the
// week's dates against the manufacturer's own calendar — see the Newsletter
// tab in the marketplace admin panel.
//
// The fingerprint covers exactly what the email asserts about each release:
// its identity, date, format, and the two flags the email prints as claims.
// Change any of those after confirming and the fingerprint stops matching,
// which re-blocks the send rather than letting an edited date ride out on a
// stale approval.
function releasesFingerprint(releases) {
  const canonical = releases
    .map((r) => [r.id, r.releaseDate, r.format, r.eql ? 'eql' : '', r.isPreorderOpenDate ? 'preorder' : ''].join('|'))
    .sort()
    .join('\n');
  return crypto.createHash('sha256').update(canonical).digest('hex').slice(0, 32);
}

// Returns null when the send may proceed, or a reason string explaining what
// has to happen first.
function dateCheckBlocker(issue) {
  const check = db.getNewsletterDateCheck.get(issue.weekOf);
  if (!check) {
    return `Release dates for the week of ${issue.weekOf} have not been confirmed yet. Open the Newsletter tab in the marketplace admin panel, check the dates against the manufacturer's calendar, and confirm them there.`;
  }
  const current = releasesFingerprint(issue.releases);
  if (check.fingerprint !== current) {
    return `The release data for the week of ${issue.weekOf} changed after it was confirmed on ${check.confirmedAt}. Re-check the dates and confirm again before sending.`;
  }
  return null;
}

// A week the owner called off from the admin panel. Deliberate, unlike an
// unconfirmed week, so it suppresses the send without raising the "the
// dates were never checked" alarm.
function weekSkip(weekOf) {
  return db.getNewsletterWeekSkip.get(weekOf) || null;
}

function dateCheckStatus(issue) {
  const check = db.getNewsletterDateCheck.get(issue.weekOf);
  const blocker = dateCheckBlocker(issue);
  const skip = weekSkip(issue.weekOf);
  return {
    weekOf: issue.weekOf,
    confirmed: !blocker,
    confirmedAt: check ? check.confirmedAt : null,
    staleSinceConfirmed: Boolean(check && blocker),
    blocker,
    skipped: Boolean(skip),
    skippedAt: skip ? skip.createdAt : null,
  };
}
...
async function sendIssueToVariant({ issue, variant, limit = MAX_PER_RUN }) {
  ...
  // Every real send passes through here — the schedule and the admin
  // endpoint alike — so this is the one place the gates have to hold.
  // Skipping outranks confirming: a week called off stays called off even
  // if its dates were checked and unlocked earlier.
  if (weekSkip(issue.weekOf)) {
    return {
      error: `The week of ${issue.weekOf} was skipped from the admin panel, so no issue is being mailed.`,
      skippedWeek: true,
    };
  }

  const blocker = dateCheckBlocker(issue);
  if (blocker) return { error: blocker, blockedOnDateCheck: true };
...
// What a send would actually do right now, per cohort — the numbers the
// admin panel's Send now button shows before anyone commits to mailing
// anyone. "remaining" is the honest count: recipients with no row for this
// issue, plus the ones whose last attempt failed, since those are the only
// addresses a run would touch. Everything else (sent, in flight, bounced,
// complained) is final for this issue and would be skipped.
function sendReadiness(issue) {
  return VARIANTS.map((variant) => {
    const recipients = recipientsForVariant(variant, issue.weekOf);
    const rows = db.listNewsletterSendStatusesForIssueVariant.all({
      issueKey: issue.issueKey,
      variant,
    });
    const statusByEmail = new Map(rows.map((row) => [row.email, row.status]));
    return {
      variant,
      eligible: recipients.length,
      sent: rows.filter((row) => row.status === 'sent').length,
      failed: rows.filter((row) => row.status === 'failed').length,
      remaining: recipients.filter((email) => {
...
async function runScheduledSend({ variant, dateISO = dateStringInTZ(new Date()), limit = MAX_PER_RUN } = {}) {
  ...
  running = true;
  try {
    // Checked before buildIssue rather than only at the send, so a skipped
    // week doesn't pay for a generated post no one is going to be mailed.
    // The blog agent still publishes on its own Sunday schedule — skipping
    // calls off the email, not the week's post.
    const weekOf = weekWindowFor(dateISO).since;
    if (weekSkip(weekOf)) {
      return { error: `The week of ${weekOf} was skipped from the admin panel, so no issue is being mailed.`, skippedWeek: true };
    }
    ...
    const result = await sendIssueToVariant({ issue, variant, limit });

    // Announced from here rather than from the schedule tick so a send
    // kicked off by hand from the admin endpoint is reported too — it's the
    // same send either way.
    await notifySendComplete(issue, variant, result);
...
function startNewsletterSchedule() {
  ...
  setInterval(() => {
    ...
    runScheduledSend({ variant, dateISO: todayISO })
      .then((result) => {
        if (result && result.skippedWeek) {
          // Deliberate, so it's a log line and not a Discord alert.
          console.log(`Weekly newsletter (${variant}): ${result.error}`);
          return;
        }
        if (result && (result.sent || result.failed || result.error)) {
          console.log('Weekly newsletter run:', JSON.stringify(result));
        }
        // A send blocked on unconfirmed dates would otherwise be a silent
        // no-op repeated every five minutes — the whole send hour could
        // pass without anyone noticing the week was skipped.
        if (result && result.blockedOnDateCheck) notifyDateCheckBlocked(variant, result.error);
      })
      .catch((err) => console.error('Weekly newsletter run failed:', err.message));
  }, TICK_INTERVAL_MS);
}

// Alerts once per (week, cohort) rather than on every tick — the schedule
// re-checks every five minutes throughout the send hour, and twelve
// identical Discord messages would train the owner to ignore them.
const blockedAlertsSent = new Set();

async function notifyDateCheckBlocked(variant, reason) {
  const today = dateStringInTZ(new Date());
  const key = `${weekWindowFor(today).since}:${variant}`;
  if (blockedAlertsSent.has(key)) return;
  blockedAlertsSent.add(key);

  console.error(`Weekly newsletter BLOCKED (${variant}): ${reason}`);

  const webhook = process.env.NEWSLETTER_WEBHOOK_URL || process.env.DISCORD_WEBHOOK_URL;
  if (!webhook) return;
  try {
    await fetch(webhook, {
      method: 'POST',
      ...
      body: JSON.stringify({
        username: 'PreorderCards Newsletter',
        embeds: [
          {
            title: `📭 Weekly newsletter held back (${variant} cohort)`,
            description: reason,
            color: 0xd9a400,
            fields: [
              {
                name: 'To send it',
                value: `Confirm this week's release dates in the Newsletter tab: ${SITE_URL}/marketplace-admin.html`,
              },
            ],
    ...
    console.error('Newsletter blocked-alert webhook failed:', err.message);
  }
}

// Posts the "it went out" notice once per issue and cohort.
//
// Held until the cohort is actually finished: a run that hits the per-run
// ceiling resumes on the next five-minute tick, and announcing after the
// first batch would report a fraction of the send as the whole of it. The
// counts come from newsletter_sends rather than the run's own counters for
// the same reason — they're the total across however many runs it took.
async function notifySendComplete(issue, variant, result) {
  if (!result || result.error || result.reachedLimit) return;

  const totals = db.newsletterSendTotalsForIssueVariant.get({ issueKey: issue.issueKey, variant });
  const sent = (totals && totals.sent) || 0;
  const failed = (totals && totals.failed) || 0;
  // A cohort with nobody in it isn't news.
  if (sent === 0 && failed === 0) return;

  // The claim is durable, so a redeploy mid-send-hour can't produce a second
  // announcement of the same issue — which would read as a double send.
  const claimed = db.claimNewsletterAlert.run({ issueKey: issue.issueKey, variant, kind: 'sent' });
  if (claimed.changes !== 1) return;

  const delivered = sent > 0;
  console.log(`Weekly newsletter ${delivered ? 'sent' : 'FAILED'} (${variant}): ${sent} delivered, ${failed} failed.`);
  ...
  const fields = [
    ...
    { name: 'Week of', value: issue.weekOf, inline: true },
  ];
  // Only worth a field when there's something in it — a clean send shouldn't
  // carry a "Failed: 0" line for the eye to check.
  if (failed > 0) fields.push({ name: 'Failed', value: String(failed), inline: true });
  if (issue.webPath) fields.push({ name: 'Post', value: `${SITE_URL}${issue.webPath}` });
  ...
  try {
    await fetch(webhook, {
      ...
      body: JSON.stringify({
        ...
        embeds: [
          {
            title: delivered
              ? `📬 Weekly newsletter sent (${variant} cohort)`
              : `⚠️ Weekly newsletter failed to send (${variant} cohort)`,
            description: issue.subject,
            color: delivered ? 0x1a7f37 : 0xb3261e,
            fields,
            timestamp: new Date().toISOString(),
          },
        ],
    ...
    console.error('Newsletter sent-alert webhook failed:', err.message);
  }
}
