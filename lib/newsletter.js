...
// 'signups' (default) = the addresses collected by the two forms that say
// out loud they're used for newsletters. 'all' additionally mails people who
// only ever registered interest in one specific release — a broader read of
// what they agreed to, so it's opt-in via the environment.
const AUDIENCE = process.env.NEWSLETTER_AUDIENCE === 'all' ? 'all' : 'signups';
...
function recipientsForVariant(variant) {
  return listRecipients().filter((email) => variantForEmail(email) === variant);
}
...
async function sendIssueToVariant({ issue, variant, limit = MAX_PER_RUN }) {
  ...
  const recipients = recipientsForVariant(variant);
  let sent = 0;
  let failed = 0;
  let skipped = 0;
...
function previewIssue(issue, { variant = 'sunday' } = {}) {
  ...
  return {
    ...
    text: message.text,
    recipientCounts: {
      sunday: recipientsForVariant('sunday').length,
      monday: recipientsForVariant('monday').length,
    },
  };
}
...
function buildReport() {
  ...
  return {
    ...
    sendHour: SEND_HOUR,
    timezone: TIMEZONE,
    listSize: listRecipients().length,
