...
// access, deferred until these run inside the interaction handler below
// (never at module top level, which Next's build step would otherwise trigger).
const db = require('./db');
...
// Stored on globalThis, not a plain module-scope variable: server.js requires
// this file directly via plain Node require() to call init() at boot, but
// Next's bundler compiles a *separate* copy of this same source file into
// each API route's bundle when a route imports it — a totally disconnected
// module instantiation with its own would-be client/ready state, since
// Turbopack's compiled chunks don't share Node's require() cache with a
// plain script. That second copy never gets init() called on it, so its
// "ready" flag would stay false forever and every alert would silently
// no-op. globalThis is a genuine process-wide singleton regardless of how
// many separate bundled copies of this source exist, so state lives there.
const state = (globalThis.__discordBotState ??= { client: null, ready: false });
...
function init({ loadReleases, sendConfirmationEmail }) {
  ...
  if (state.client) return; // already initialized (e.g. a hot-reload re-run)

  state.client = new Client({ intents: [GatewayIntentBits.Guilds] });
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    const row = db.getInterestById.get(id);
    ...
    db.markEmailSent.run({ id, sentAt });
...
async function postInterestAlert(release, row) {
  if (!state.ready || !CHANNEL_ID) return false;
  try {
