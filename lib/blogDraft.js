...
const { loadListableReleases, hasKnownDate, todayISO } = require('./releases');
...
function getReleasesInWindow(sinceISO, untilISO) {
  ...
  // hasKnownDate first: a release with no announced date belongs to no week,
  // so it never reaches the newsletter or the blog agent until it gets one.
  return data.releases
    .filter((r) => hasKnownDate(r) && r.releaseDate >= sinceISO && r.releaseDate <= untilISO)
