...
// Brands the calendar tracks. Anything else in the data is a typo, and the
// check script flags it rather than letting it render as a mystery badge.
const MANUFACTURERS = ['Topps', 'Panini'];
const DEFAULT_MANUFACTURER = 'Topps';

// Two categories the site deliberately doesn't list.
//
// Women's sports: matched on the title, since that's where the league or
// competition name lives. Kept as an explicit word-boundary list rather than
// a loose substring match so it can't catch an unrelated product — "Women's"
// and the women's-league acronyms only.
const WOMENS_RE = /\b(women'?s|wnba|nwsl|w-?league|aflw|wsl)\b/i;

// Dutch auctions: not inferable from a title — Topps runs "First Day Issue"
// Dutch auctions for products whose ordinary hobby release is listed
// normally — so this is a flag set per entry in the data.
function exclusionReason(release) {
  if (release.dutchAuction) return 'dutch-auction';
  if (WOMENS_RE.test(release.title || '') || WOMENS_RE.test(release.description || '')) return 'womens';
  return null;
}

function isListable(release) {
  return exclusionReason(release) === null;
}

// Some products are announced with no date at all — Panini's calendar lists
// them as "coming soon" with nothing more. They're real and worth listing,
// but inventing a date for them would put a false claim on the card and in
// the newsletter, so they carry dateTbd instead and sort to the end.
function hasKnownDate(release) {
  return typeof release.releaseDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(release.releaseDate);
}

// Undated releases sort after every dated one, in title order among
// themselves — a stable order, rather than whatever the file happens to hold.
...
// The raw file. Used where an id has to resolve no matter what — the admin
// panel showing existing registrations, and the outcome emails that go with
// them. Someone who registered before a release was excluded is still owed
// an answer, and that lookup must not start failing.
function loadReleases() {
  const raw = fs.readFileSync(RELEASES_PATH, 'utf8');
  const data = JSON.parse(raw);
  data.releases = data.releases.map((r) => ({
    ...r,
    manufacturer: r.manufacturer || DEFAULT_MANUFACTURER,
  }));
  return data;
}

// What the site is willing to put in front of people: the homepage, the API,
// the newsletter, the blog agent, and new interest registrations. Excluded
// releases are absent from all of them.
function loadListableReleases() {
  const data = loadReleases();
  return { ...data, releases: data.releases.filter(isListable) };
}
...
async function sendConfirmationEmail(release, { contactType, contactValue, quantity }) {
  ...
  return sendEmail({
    ...
    text: [
      ...
      '',
      "You'll also start getting our free weekly release roundup — one email a week covering what's dropping. It has an unsubscribe link at the bottom, or just reply here and we'll take you off it.",
      '',
      '— PreorderCards',
      '',
    ...
    html: `
      ...
      <p>You'll also start getting our free <strong>weekly release roundup</strong> — one email a
      week covering what's dropping. It has an unsubscribe link at the bottom, or just reply here
      and we'll take you off it.</p>
      <p>— PreorderCards</p>
